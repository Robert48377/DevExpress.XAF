...
workflow Install-AllDXNugets{
    ...
    Foreach -parallel ($nuget in $psObj.Nugets)    { 
        InlineScript {
            ...
            $res=& $Using:psObj.NugetExe Install $Using:nuget.Name -source "$($Using:psObj.Source);https://xpandnugetserver.azurewebsites.net/nuget" -OutputDirectory $Using:psObj.OutputDirectory
            if ($LASTEXITCODE) {
                throw $res
            }
        } 
